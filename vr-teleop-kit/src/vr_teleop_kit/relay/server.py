...
class CameraReader:
    ...
    def __init__(self, spec: CameraSpec) -> None:
        if cv2 is None:
            raise RuntimeError("WebRTC camera support requires opencv-python-headless")
        self.spec = spec
...
class CameraTrack(VideoStreamTrack):
    ...
    async def recv(self) -> av.VideoFrame:
        if av is None:
            raise RuntimeError("WebRTC camera support requires av")
...
async def _handle_webrtc_request(state: ClientState, msg: dict) -> None:
    if not WEBRTC_AVAILABLE:
        await state.ws.send_text(json.dumps({
            "type": "webrtc_error",
            "message": (
                "WebRTC video dependencies are not installed. "
                "Install vr-teleop-kit[relay] with dependencies if camera streaming is needed."
            ),
            "detail": str(WEBRTC_IMPORT_ERROR),
        }))
        return
...
async def _handle_webrtc_answer(state: ClientState, msg: dict) -> None:
    ...
    if RTCSessionDescription is None:
        logger.warning("webrtc_answer ignored because WebRTC dependencies are missing")
        return
