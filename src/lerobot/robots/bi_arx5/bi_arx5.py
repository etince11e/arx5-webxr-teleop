...
def _has_unified_tcp_action(action: dict[str, Any], prefix: str) -> bool:
    keys = (*_prefixed_keys(prefix, _BI_CARTESIAN_TCP_KEYS), *_prefixed_keys(prefix, _BI_CARTESIAN_R6D_KEYS))
    return any(k in action for k in keys)


def _tcp_offset_for_prefix(config: BiARX5Config, prefix: str) -> Sequence[float]:
    if prefix == "left":
        return config.left_tcp_offset_xyz
    if prefix == "right":
        return config.right_tcp_offset_xyz
    raise ValueError(f"Unsupported arm prefix: {prefix!r}")
...
class BiARX5(Robot):
    ...
    def _apply_gripper_robot_config(self) -> None:
        ...
        for robot_config in self.robot_configs.values():
            robot_config.gripper_vel_max = gripper_vel_max
    ...
    def _apply_gripper_controller_config(self) -> None:
        ...
        if mode == "mit":
            ...
            for controller_config in self.controller_configs.values():
                controller_config.default_gripper_kp = gripper_kp
                controller_config.default_gripper_kd = gripper_kd

        for controller_config in self.controller_configs.values():
            controller_config.over_current_cnt_max = over_current_cnt_max

        left_cfg = self.controller_configs["left_config"]
        left_robot_cfg = self.robot_configs["left_config"]
        self.logger.info(
            "Configured gripper control | "
            f"mode={mode} | "
            f"kp={left_cfg.default_gripper_kp:.3f} | "
            f"kd={left_cfg.default_gripper_kd:.3f} | "
            f"vel_max={left_robot_cfg.gripper_vel_max:.3f} | "
            f"torque_max={left_robot_cfg.gripper_torque_max:.3f} | "
            f"over_current_cnt_max={left_cfg.over_current_cnt_max}"
        )

    def _apply_gripper_gain(self, gain: Any, controller_config: Any) -> None:
        gain.gripper_kp = float(controller_config.default_gripper_kp)
        gain.gripper_kd = float(controller_config.default_gripper_kd)
    ...
    def connect(self, calibrate: bool = False, go_to_start: bool = True) -> None:
        ...
        if self.config.control_mode == BiARX5ControlMode.CARTESIAN_CONTROL:
            ...
            self._left_eef_cmd_buffer = arx5.EEFState(left_state.pose_6d(), left_state.gripper_pos)
            self._right_eef_cmd_buffer = arx5.EEFState(right_state.pose_6d(), right_state.gripper_pos)
            self._apply_gripper_command_fields(self._left_eef_cmd_buffer)
            self._apply_gripper_command_fields(self._right_eef_cmd_buffer)
            ...
            self._left_cmd_buffer = arx5.JointState(
                left_state.pos().copy(),
                np.zeros(self.robot_configs["left_config"].joint_dof, dtype=np.float64),
                np.zeros(self.robot_configs["left_config"].joint_dof, dtype=np.float64),
                left_state.gripper_pos,
            )
            self._right_cmd_buffer = arx5.JointState(
                right_state.pos().copy(),
                np.zeros(self.robot_configs["right_config"].joint_dof, dtype=np.float64),
                np.zeros(self.robot_configs["right_config"].joint_dof, dtype=np.float64),
                right_state.gripper_pos,
            )
            self._apply_gripper_command_fields(self._left_cmd_buffer)
            self._apply_gripper_command_fields(self._right_cmd_buffer)
    ...
    def get_start_eef_pose(self) -> np.ndarray:
        ...
        prefix = self._spacemouse_arm_side()
        start_tcp_pose = _eef_pose_to_tcp_pose(
            self._start_position_eef[:6],
            _tcp_offset_for_prefix(self.config, prefix),
        )
    ...
    def get_current_tcp_pose_euler(self) -> np.ndarray:
        ...
        prefix = self._spacemouse_arm_side()
        arm = self.left_arm if prefix == "left" else self.right_arm
        eef_state = arm.get_eef_state()
        eef_pose_6d = np.asarray(eef_state.pose_6d(), dtype=np.float64).reshape(6)
        tcp_pose_6d = _eef_pose_to_tcp_pose(eef_pose_6d, _tcp_offset_for_prefix(self.config, prefix))
    ...
    def get_current_tcp_poses_euler(self) -> tuple[np.ndarray, np.ndarray]:
        """Current TCP poses for both arms in Euler form, including grippers."""
        if not self._is_connected:
            raise DeviceNotConnectedError(f"{self} is not connected.")
        if self.config.control_mode != BiARX5ControlMode.CARTESIAN_CONTROL:
            raise ValueError("get_current_tcp_poses_euler requires CARTESIAN_CONTROL mode")

        def _read_arm(arm, prefix: str) -> np.ndarray:
            eef_state = arm.get_eef_state()
            eef_pose_6d = np.asarray(eef_state.pose_6d(), dtype=np.float64).reshape(6)
            tcp_pose_6d = _eef_pose_to_tcp_pose(eef_pose_6d, _tcp_offset_for_prefix(self.config, prefix))
            gripper_pos = float(eef_state.gripper_pos)
        ...
        return _read_arm(self.left_arm, "left"), _read_arm(self.right_arm, "right")
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if self.config.control_mode == BiARX5ControlMode.CARTESIAN_CONTROL:
            # SDK returns eef_link Euler per arm; publish the calibrated real
            # TCP as the unified left_tcp.* / right_tcp.* + 6D rotation schema.
            left_eef_state = self.left_arm.get_eef_state()
            left_eef_pose_xyzrpy = np.asarray(left_eef_state.pose_6d(), dtype=np.float64).reshape(6)
            left_tcp_pose_xyzrpy = _eef_pose_to_tcp_pose(
                left_eef_pose_xyzrpy,
                self.config.left_tcp_offset_xyz,
            )
            obs_dict.update(_euler_pose_to_tcp_6d(left_tcp_pose_xyzrpy, prefix="left"))
            obs_dict[f"left_{_BI_CARTESIAN_GRIPPER_KEY}"] = float(left_eef_state.gripper_pos)

            right_eef_state = self.right_arm.get_eef_state()
            right_eef_pose_xyzrpy = np.asarray(right_eef_state.pose_6d(), dtype=np.float64).reshape(6)
            right_tcp_pose_xyzrpy = _eef_pose_to_tcp_pose(
                right_eef_pose_xyzrpy,
                self.config.right_tcp_offset_xyz,
            )
            obs_dict.update(_euler_pose_to_tcp_6d(right_tcp_pose_xyzrpy, prefix="right"))
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.control_mode == BiARX5ControlMode.CARTESIAN_CONTROL:
            # External left_tcp.* / right_tcp.* commands target each calibrated
            # real TCP. Legacy left_x/y/z/roll/pitch/yaw keys are internal EEF
            # trajectory commands and are intentionally not TCP-offset converted.
            for prefix, cmd, arm in (
                ...
                if _has_unified_tcp_action(action, prefix):
                    tcp_offset_xyz = _tcp_offset_for_prefix(self.config, prefix)
                    fallback_tcp_pose = _eef_pose_to_tcp_pose(fallback, tcp_offset_xyz)
                    target_tcp_pose = np.asarray(
                        _tcp_6d_action_to_euler(action, prefix, fallback_tcp_pose),
                        dtype=np.float64,
                    )
                    target_eef_pose = _tcp_pose_to_eef_pose(target_tcp_pose, tcp_offset_xyz)
                else:
                    target_eef_pose = np.asarray(
                        _tcp_6d_action_to_euler(action, prefix, fallback),
                        dtype=np.float64,
                    )
            ...
            self._apply_gripper_command_fields(left_cmd)
            ...
            self._apply_gripper_command_fields(right_cmd)
    ...
    def set_to_normal_position_control(self):
        ...
        if self._is_gravity_compensation_mode and is_joint_mode:
            ...
            left_gain.kp()[:] = left_cfg.default_kp * 0.5
            left_gain.kd()[:] = left_cfg.default_kd * 1.5
            self._apply_gripper_gain(left_gain, left_cfg)

            right_cfg = self.controller_configs["right_config"]
            right_gain = self.right_arm.get_gain()
            right_gain.kp()[:] = right_cfg.default_kp * 0.5
            right_gain.kd()[:] = right_cfg.default_kd * 1.5
            self._apply_gripper_gain(right_gain, right_cfg)
    ...
    def smooth_go_start(self, duration: float | None = None, easing: str = "ease_in_out_quad") -> None:
        ...
        if self.config.control_mode == BiARX5ControlMode.CARTESIAN_CONTROL:
            ...
            self._apply_gripper_command_fields(current_left_cmd)
            ...
            self._apply_gripper_command_fields(current_right_cmd)
